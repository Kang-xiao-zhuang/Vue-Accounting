...
export default {
  ...
  restore(data, mode) {
    return http.post('/backup/restore', data, { params: { mode } }).then(r => r.data)
  },
  ...
  // ---- per-user key/value state (cross-device) ----
  getState(key) {
    return http.get('/state/' + key).then(r => (r.data ? r.data.content : null))
  },
  putState(key, content) {
    return http.put('/state/' + key, { content })
  }
}
